...
-- 2) Check code (audit + export JSON with summary including paths)
		...
		if inst:IsA("Script") or inst:IsA("LocalScript") or inst:IsA("ModuleScript") then
			...
			if src then
				...
				if #findings > 0 then
					...
					-- Update summary with this script's severities
					local seenSeverities = {} -- avoid duplicating same script in summary
...
local function saveAuditReport(reportTable)
	...
	local old = ServerStorage:FindFirstChild("AuditReport")
	...
	warn("[Audit] Report saved in ServerStorage/AuditReport (Right-click -> Save to File…) ")
end
